...
public:
    ...
    std::vector<Position> find_adjacent_non_visited_indices_from_position(const Position& position, const std::unordered_set<Position>& visited_positions) const override;

private:
    bool is_position_already_added(const Position& position, const std::vector<Position>& adjacent_positions) const;
    bool was_position_already_visited(const Position& position, const std::unordered_set<Position>& visited_positions) const;
